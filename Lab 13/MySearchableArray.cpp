// Lab 13, SeachableArray Template
...
int main()
{
  SearchableArray<double>arr(100);
  SearchableArray<bool>index_value(100);
  ...
  while (true)
  {
    int value, index;
    string buf;
    cout << endl << "Input a value (within the sorted range) for me to look up [Q to quit]: ";
    cin>> buf;
    if (buf == "Q" || buf == "q") break;
    else
      value = atoi(buf.c_str());
    index = arr.bsearch(element, value);
    if (index < 0)
